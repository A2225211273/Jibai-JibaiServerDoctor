 ...
 *
 * <p>负责加载可自定义的提示文本，支持 {@code {key}} 占位符替换，并通过 {@link ColorUtil}
 * 把颜色标签 / &amp; 码解析为 Bukkit {@link org.bukkit.ChatColor}。所有对外发送方法输出的
 * 都是已上色的普通字符串，兼容 Bukkit / Spigot / Paper / Purpur 全核心与控制台。</p>
 *
 * @author 即白
 */
public class MessageManager {
    ...
    public MessageManager(JibaiOptimizerPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    /**
     * 获取原始字符串（未解析颜色、未加前缀），路径不存在时返回路径本身以便排查。
     */
    ...
    public String raw(String path) {
        return raw(path, Collections.emptyMap());
    }
    ...
    public String withPrefix(String path) {
        return withPrefix(path, Collections.emptyMap());
    }
    ...
    public void send(CommandSender target, String path, Map<String, String> placeholders) {
        target.sendMessage(withPrefix(path, placeholders));
    }
    ...
    public void sendRaw(CommandSender target, String path, Map<String, String> placeholders) {
        target.sendMessage(raw(path, placeholders));
    }
